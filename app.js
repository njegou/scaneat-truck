...
const ScanEatApp = () => {
  ...
  const foodItems = ['🥗','🍝','🍅','🌿','🌶️','🐟','🍣','🍗','🥑','🥬','🧄','🫒','🧀','🥚','🍋','🌾','🥕','🫑','🧅','🐐','🍹','💧'];
  const bgParticles = useMemo(() => Array.from({length: 22}, (_, i) => ({
    id: i,
  ...
  const FoodBackground = ({ children, overlay = 'rgba(255,255,255,0.88)' }) => (
    <div style={{ position: 'relative', minHeight: '100vh', background: '#fffaf5', overflow: 'hidden' }}>
      <style>{`
        @keyframes floatBob {
          0%   { transform: translateY(0px) rotate(var(--rot)); opacity: 0.18; }
          50%  { transform: translateY(-14px) rotate(calc(var(--rot) + 5deg)); opacity: 0.28; }
          100% { transform: translateY(0px) rotate(var(--rot)); opacity: 0.18; }
        }
      `}</style>
      {/* Pattern ingrédients */}
      ...
      {/* Voile coloré par-dessus les emojis */}
