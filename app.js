...
const ScanEatApp = () => {
  ...
  // Simulation des réservations (Capacité max: 8 par créneau)
  const [bookings] = useState({
    "12:15": 8, // COMPLET (Bouton désactivé)
    "12:30": 7, // LIMITE ATTEINTE (Animation VITE !)
    "13:00": 3
  });
  ...
  // --- GÉNÉRATION CRÉNEAUX (11:30 - 14:30) ---
    ...
    <FoodBackground photo="visuel_scaneat_truck.jpg">
      <div className="min-h-screen p-8 flex flex-col justify-center text-center">
        ...
        <div className="grid gap-4">
          <button onClick={() => {setSelectedFormule('solo'); setStep('choix-plat');}} className="p-8 border-2 border-white/10 rounded-[40px] text-left hover:border-orange-500 bg-white/5 transition-all text-white">
          ...
          <button onClick={() => {setSelectedFormule('trio'); setStep('choix-entree');}} className="p-8 border-2 border-white/10 rounded-[40px] text-left hover:border-orange-500 bg-white/5 text-white/50">
            <h3 className="text-xl font-black">ENTRÉE + PLAT + BOISSON</h3>
    ...
    return (
      <FoodBackground photo="visuel_scaneat_truck_2.jpg">
        <div className="min-h-screen p-8 flex flex-col text-white">
          <h2 className="text-3xl font-black italic uppercase mb-8">{title}</h2>
          <div className="grid gap-3 overflow-y-auto max-h-[75vh]">
            {items.map(item => (
              <button key={item.id} onClick={() => { setSelections({...selections, [cat]: item}); 
                if(cat === 'entree') setStep('choix-plat');
                else if(cat === 'plat') setStep('choix-boisson');
                else if(cat === 'boisson') selectedFormule === 'trio' ? setStep('choix-dessert') : setStep('recap');
                else if(cat === 'dessert') setStep('recap');
              }} className="bg-white/5 p-5 rounded-[30px] flex items-center gap-5 border border-white/10 hover:border-orange-500 transition-all text-left">
                <span className="text-4xl w-16 h-16 flex items-center justify-center bg-white/5 rounded-2xl">{item.image}</span>
                <div>
                  <h4 className="font-black uppercase text-xs tracking-wider">{item.name}</h4>
    ...
    <FoodBackground photo="visuel_scaneat_truck.jpg">
      <div className="min-h-screen p-8 flex flex-col justify-center text-center">
        ...
        <div className="bg-gray-900/90 text-white p-8 rounded-[40px] mb-8 text-left border border-gray-800">
            <p className="text-4xl font-black mb-6 border-b border-gray-800 pb-4 text-orange-500 tracking-tighter">{prixFormules[selectedFormule]} €</p>
        ...
        <input type="text" placeholder="ENTREZ VOTRE PRÉNOM" className="w-full bg-white/5 p-6 rounded-3xl font-black mb-8 border-2 border-white/10 focus:border-orange-500 outline-none text-center text-white" value={customerName} onChange={(e) => setCustomerName(e.target.value)} />
        {/* LE BOUTON DÉCLENCHEUR POUR LES CRÉNEAUX */}
        <button disabled={!customerName} onClick={() => setStep('choix-horaire')} className="w-full bg-orange-500 text-white p-6 rounded-[35px] font-black text-xl uppercase italic shadow-xl disabled:opacity-20">Confirmer et Payer</button>
      </div>
    </FoodBackground>
  );
  ...
  if (step === 'choix-horaire') return (
    <FoodBackground photo="visuel_scaneat_truck_2.jpg">
      <div className="min-h-screen p-8 flex flex-col text-center">
        ...
        <p className="text-white/40 text-[10px] uppercase font-bold tracking-widest mb-10">Choisir un créneau disponible (Max 8 commandes)</p>
        <div className="grid grid-cols-3 gap-3">
          {timeSlots.map(time => {
            const count = bookings[time] || 0;
            const isFull = count >= 8;
            const isUrgent = count === 7;
            return (
              <button key={time} disabled={isFull} onClick={() => { setSelectedTime(time); setStep('paiement'); }}
                className={`p-4 rounded-3xl border-2 transition-all relative flex flex-col items-center ${
                  isFull ? 'opacity-20 border-white/5 bg-white/5 cursor-not-allowed' : 'border-white/10 bg-white/10 text-white hover:border-orange-500'
                }`}>
                <span className="text-sm font-black">{time}</span>
                {isUrgent && <span className="absolute -top-2 bg-red-600 text-white text-[7px] px-2 py-0.5 rounded-full animate-pulse font-black uppercase">VITE !</span>}
  ...
  if (step === 'confirmation') return (
    <div className="min-h-screen bg-orange-500 flex items-center justify-center p-8 text-center text-gray-900">
        <div className="bg-white rounded-[60px] p-12 w-full shadow-2xl max-w-sm border-b-8 border-orange-600">
            <h2 className="text-3xl font-black mb-4 uppercase italic">C'EST TOUT BON !</h2>
            <p className="text-gray-400 text-xs font-bold mb-10 uppercase">Rendez-vous à {selectedTime} au ScanEat Truck</p>
            ...
            <button onClick={() => window.location.reload()} className="mt-10 text-gray-300 font-bold uppercase text-[9px] tracking-widest">Nouvelle Commande</button>
        </div>
    </div>
  );
  return null;
