...
const ScanEatApp = () => {
  ...
  const Header = ({ showBack }) => (
    <div className="sticky top-0 z-50 bg-white/90 backdrop-blur-md p-5 border-b flex items-center justify-between shadow-sm">
      ...
      ) : <h1 className="font-black italic text-xl uppercase tracking-tighter text-gray-800">ScanEat<span className="text-orange-500">.</span></h1>}
      <div className="w-10" />
    </div>
  );
        ...
        {Object.entries(menuItems).map(([cat, items]) => (
          <section key={cat}>
            ...
            <div className="grid gap-3">
              {items.map(item => (
                <div key={item.id} className="bg-white p-4 rounded-2xl shadow-sm flex items-center justify-between active:scale-95 transition-transform border border-gray-100">
  ...
  const TimePage = () => (
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="p-6 text-center">
        <h2 className="text-2xl font-black my-8 uppercase italic text-gray-800">Horaire</h2>
  ...
  const PaymentPage = () => (
    <div className="min-h-screen bg-slate-900 flex items-center justify-center p-6 text-center">
      <div className="bg-white rounded-[40px] p-8 w-full max-w-sm">
        <h2 className="text-4xl font-black mb-10 text-gray-900">{calculateTotal.final} €</h2>
        {isPaying ? <div className="animate-spin w-10 h-10 border-4 border-orange-500 border-t-transparent rounded-full mx-auto mb-4"></div> : 
  ...
  const ConfirmationPage = () => (
    <div className="min-h-screen bg-orange-500 flex items-center justify-center p-6 text-center">
      <div className="bg-white rounded-[50px] p-10 w-full max-w-sm shadow-2xl">
        <h2 className="text-2xl font-black mb-6 text-gray-900">Merci {customerName} !</h2>
        <div className="bg-slate-50 border-2 border-dashed border-gray-200 rounded-[35px] p-8 mb-8">
          <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block mb-2">Code de Retrait</span>
        ...
        <button onClick={() => window.location.reload()} className="text-gray-400 font-bold text-[10px] uppercase tracking-widest">Retour au menu</button>
      </div>
    </div>
  );
