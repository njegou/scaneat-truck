...
const ScanEatApp = () => {
  ...
  const FoodBackground = ({ children, photo = 'visuel_scaneat_truck.png' }) => (
  ...
  if (step === 'selection-formule') return (
    <FoodBackground>
      <div className="min-h-screen p-8 flex flex-col justify-center">
        <h1 className="text-4xl font-black mb-2 italic uppercase text-center text-white">ScanEat<span className="text-orange-500">.</span></h1>
        <p className="text-center text-orange-300/60 font-bold uppercase text-[10px] tracking-[0.3em] mb-12">Borne de commande digitale</p>
        <div className="grid gap-4">
          <button onClick={() => {setSelectedFormule('solo'); setStep('choix-plat');}} className="p-8 border-2 border-white/10 rounded-[40px] text-left hover:border-orange-500 transition-all bg-white/10 backdrop-blur-md">
            <span className="text-[10px] font-black text-orange-400 uppercase">Solo</span>
            <h3 className="text-xl font-black text-white">PLAT SEUL</h3>
            <p className="text-2xl font-black mt-2 text-white">9.50 €</p>
          </button>
          <button onClick={() => {setSelectedFormule('duo'); setStep('choix-plat');}} className="p-8 bg-orange-500 text-white rounded-[40px] text-left relative shadow-2xl">
            <div className="absolute top-4 right-6 bg-white text-orange-500 px-3 py-1 rounded-full text-[9px] font-black uppercase">Populaire</div>
            <span className="text-[10px] font-black text-white/70 uppercase">Duo</span>
            <h3 className="text-xl font-black">PLAT + BOISSON</h3>
            <p className="text-2xl font-black mt-2">11.50 €</p>
          </button>
          <button onClick={() => {setSelectedFormule('trio'); setStep('choix-plat');}} className="p-8 border-2 border-white/10 rounded-[40px] text-left hover:border-orange-500 transition-all bg-white/10 backdrop-blur-md">
            <span className="text-[10px] font-black text-orange-400 uppercase">Trio</span>
            <h3 className="text-xl font-black text-white">PLAT + BOISSON + DESSERT</h3>
            <p className="text-2xl font-black mt-2 text-white">14.90 €</p>
          </button>
        </div>
      </div>
    </FoodBackground>
    ...
    <FoodBackground photo="visuel_scaneat_truck_2.png">
      <div className="min-h-screen flex flex-col">
        <div className="p-8 pb-4">
          <h2 className="text-3xl font-black text-white leading-tight mb-2 italic uppercase">{title}</h2>
          <div className="h-1 w-20 bg-orange-500 rounded-full mb-8"></div>
          <div className="grid gap-3 overflow-y-auto max-h-[70vh] pr-2">
            {items.map(item => (
              <button key={item.id} onClick={() => selectItem(category, item)} className="bg-white/10 backdrop-blur-md p-5 rounded-[30px] flex items-center gap-5 hover:bg-white/20 active:scale-95 transition-all text-left border border-white/10 hover:border-orange-500">
                <span className="text-4xl bg-white/10 w-16 h-16 flex items-center justify-center rounded-2xl">{item.image}</span>
                <div>
  ...
  if (step === 'recap') return (
    <FoodBackground>
      <div className="min-h-screen p-8">
        ...
        <div className="bg-orange-500 text-white p-8 rounded-[40px] mb-8 shadow-2xl">
            <div className="flex justify-between items-center mb-6 border-b border-orange-400 pb-4">
                <span className="text-[10px] font-bold text-white/70 uppercase tracking-[0.2em]">Total Formule</span>
                <span className="text-3xl font-black tracking-tighter">{prixFormules[selectedFormule]} €</span>
            </div>
            <div className="space-y-4">
                <div className="flex items-center gap-3"><span className="text-white">✔</span> <span className="font-bold text-sm uppercase">{selections.plat?.name}</span></div>
                {selections.boisson && <div className="flex items-center gap-3"><span className="text-white">✔</span> <span className="font-bold text-sm uppercase">{selections.boisson?.name}</span></div>}
                {selections.dessert && <div className="flex items-center gap-3"><span className="text-white">✔</span> <span className="font-bold text-sm uppercase">{selections.dessert?.name}</span></div>}
            </div>
        </div>
        <input type="text" placeholder="VOTRE PRÉNOM" className="w-full bg-white/10 backdrop-blur-md text-white placeholder-white/40 p-6 rounded-3xl font-black mb-8 border-2 border-white/10 focus:border-orange-500 outline-none text-center uppercase tracking-widest" value={customerName} onChange={(e) => setCustomerName(e.target.value)} />
        <button disabled={!customerName} onClick={() => setStep('paiement')} className="w-full bg-orange-500 text-white p-6 rounded-[35px] font-black text-xl shadow-xl disabled:opacity-30 transition-all uppercase italic">Valider la commande</button>
      </div>
    </FoodBackground>
  );
